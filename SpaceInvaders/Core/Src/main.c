...
#include "task.h"
#include "stm32f4xx_hal_adc.h" // Thêm để định nghĩa ADC_HandleTypeDef
...
/* Định nghĩa ngưỡng ADC cho joystick */
#define JOYSTICK_X_LEFT_THRESHOLD   1000  // Giá trị ADC trục X nhỏ hơn ngưỡng này là hướng trái
#define JOYSTICK_X_RIGHT_THRESHOLD  3000  // Giá trị ADC trục X lớn hơn ngưỡng này là hướng phải
#define JOYSTICK_Y_UP_THRESHOLD     1000  // Giá trị ADC trục Y nhỏ hơn ngưỡng này là hướng lên
#define JOYSTICK_Y_DOWN_THRESHOLD   3000  // Giá trị ADC trục Y lớn hơn ngưỡng này là hướng xuống
/* USER CODE END PD */
...
ADC_HandleTypeDef hadc1; // Biến cấu hình ADC cho joystick
...
static void MX_ADC1_Init(void); // Hàm khởi tạo ADC cho joystick
...
uint32_t I2c3Timeout = I2C3_TIMEOUT_MAX;
uint32_t Spi5Timeout = SPI5_TIMEOUT_MAX;
/* USER CODE END 0 */
...
static void MX_ADC1_Init(void)
{
  ...
  // Cấu hình ADC1: độ phân giải 12-bit, chuyển đổi tuần tự
  hadc1.Instance = ADC1;
  ...
  /* Cấu hình kênh ADC cho trục X (PA0 - ADC1_IN0) */
  ...
  /* Cấu hình kênh ADC cho trục Y (PA1 - ADC1_IN1) */
...
static void MX_GPIO_Init(void)
{
  ...
  /* Cấu hình chân PE2 làm ngõ vào cho nút bấm của joystick (nếu có) */
  GPIO_InitStruct.Pin = GPIO_PIN_2;
...
void IOE_Init(void)
{
  ...
  // This is done with cubmx and is therefore not done here.
}
...
void IOE_ITConfig(void)
{
  // Dummy function called when initializing to stmpe811 to setup interrupt for the i2c.
  // The interrupt is not used in our case, therefore nothing is done here.
}
